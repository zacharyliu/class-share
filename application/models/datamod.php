...
class Datamod extends CI_Model {
    ...
    public function search($q) {
        ...
        $query = $this->db->query("SELECT * FROM (SELECT id, CONCAT(name, ', ', teacher) AS body, 'class' AS type FROM classes UNION ALL SELECT people.id, name AS body, 'person' AS type FROM people INNER JOIN enrollment ON people.id = enrollment.user_id) AS search WHERE body LIKE '%$q%' GROUP BY body");
        $result = $query->result();
